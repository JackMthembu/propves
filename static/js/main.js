(function() {
  ...
  // Improved resize observer implementation
  const resizeObserver = new ResizeObserver(entries => {
    entries.forEach(entry => {
      try {
        const charts = echarts.getInstanceByDom(entry.target);
  ...
  // Only observe elements that exist and have charts initialized
  const chartElements = document.querySelectorAll('.echart');
  chartElements.forEach(element => {
    if (element && echarts.getInstanceByDom(element)) {
...
function previewImage(event) {
  ...
  reader.onload = function() {
      if (reader.readyState === 2) {
          profilePic.src = reader.result;  // Update the profile picture with the new file
      }
  };
...
function validatePhoneNumber(input) {
  ...
  if (phoneNumberPattern.test(phoneInput.value)) {
      ...
      phoneError.innerHTML = 'Invalid phone number format. Please use international format (e.g., +27123456789).';
  }
}